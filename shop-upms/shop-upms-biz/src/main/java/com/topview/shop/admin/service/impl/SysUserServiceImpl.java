...
import com.topview.shop.admin.api.entity.*;
import com.topview.shop.admin.api.util.ParamResolver;
...
import com.topview.shop.common.core.constant.enums.MenuTypeEnum;
...
import com.topview.shop.admin.mapper.*;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
	...
	public CommonResult<Boolean> updateUserInfo(UserDTO userDto) {
		...
		if (!StrUtil.equals(userVO.getPhone(), userDto.getPhone())) {
			if (!appService.check(userDto.getPhone(), userDto.getCode())) {
				return CommonResult.failed(MsgUtils.getMessage(ErrorCodes.SYS_APP_SMS_ERROR));
			}
		}
		...
		return CommonResult.ok(this.updateById(sysUser));
	}
	...
	public CommonResult<Boolean> registerUser(UserDTO userDto) {
		...
		if (sysUser != null) {
			return CommonResult
					.failed(MsgUtils.getMessage(ErrorCodes.SYS_USER_USERNAME_EXISTING, userDto.getUsername()));
		}
		...
		if (sysRole == null) {
			return CommonResult.failed(MsgUtils.getMessage(ErrorCodes.SYS_PARAM_CONFIG_ERROR, "USER_DEFAULT_ROLE"));
		}

		userDto.setRole(Collections.singletonList(sysRole.getRoleId()));
		return CommonResult.ok(saveUser(userDto));
	}
