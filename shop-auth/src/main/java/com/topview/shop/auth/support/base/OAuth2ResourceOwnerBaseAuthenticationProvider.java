package com.topview.shop.auth.support.base;

import cn.hutool.extra.spring.SpringUtil;

import com.topview.shop.common.security.util.OAuth2ErrorCodesExpand;
import com.topview.shop.common.security.util.ScopeException;
...
public abstract class OAuth2ResourceOwnerBaseAuthenticationProvider<T extends OAuth2ResourceOwnerBaseAuthenticationToken>
	...
	/**
	 * 使用与{@link AuthenticationManager#authenticate(Authentication)}相同的约定执行身份验证
	 * @param authentication the authentication request object.
	 * 返回包含凭据的完全身份验证对象。如果AuthenticationProvider无法支持传递的authentication对象的身份验证，则可能返回null。
	 * 在这种情况下，将尝试下一个支持所呈现的Authentication类的AuthenticationProvider
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		try {
			...
			DefaultOAuth2TokenContext.Builder tokenContextBuilder = DefaultOAuth2TokenContext.builder()
					.registeredClient(registeredClient).principal(usernamePasswordAuthentication)
					.authorizationServerContext(AuthorizationServerContextHolder.getContext())
					.authorizedScopes(authorizedScopes).authorizationGrantType(AuthorizationGrantType.PASSWORD)
			...
			if (generatedAccessToken == null) {
				OAuth2Error error = new OAuth2Error(OAuth2ErrorCodes.SERVER_ERROR, "令牌生成器无法生成访问令牌accessToken.",
						ERROR_URI);
	...
	/**
	 * 判断客户端是否授权成功 授权成功强转Authentication对象，否则抛出异常‘无效客户端’
	 */
