...
/**
 * 简化{@code CommonResult<T>} 的访问操作,例子 CommonResult<Integer> result = CommonResult.ok(0);
 *
 * 使用场景1: 链式操作: 断言然后消费 RetOps.of(result) .assertCode(-1,r -> new RuntimeException("error
 * "+r.getCode())) .assertDataNotEmpty(r -> new IllegalStateException("oops!"))
 * .useData(System.out::println);
 *
 * // 使用场景2: 读取原始值(data),这里返回的是Optional RetOps.of(result).getData().orElse(null);
 *
 * // 使用场景3: 类型转换 R<String> s = RetOps.of(result) .assertDataNotNull(r -> new
 * IllegalStateException("nani??")) .map(i -> Integer.toHexString(i)) .peek();
 *
 * @author admin
 */
public class RetOps<T> {
	 ...
	 * 状态码为成功
	 */
	public static final Predicate<CommonResult<?>> CODE_SUCCESS = r -> CommonConstants.SUCCESS == r.getCode();
	...
	public <U> RetOps<U> map(Function<? super T, ? extends U> mapper) {
		CommonResult<U> result = CommonResult.restResult(mapper.apply(original.getData()), original.getCode(),
				original.getMessage());
		return of(result);
	}
